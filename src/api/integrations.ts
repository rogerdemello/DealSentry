...
router.delete('/:id', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    // SyncLog rows reference this integration; without a DB-level cascade the
    // delete fails for any integration that has ever synced.
...
router.post('/:id/sync', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    // Demo-mode connections (credentials.demo, set by the *_DEMO_MODE OAuth
    // flows) import canned deals so connect → sync → review is fully
    // demonstrable without real CRM credentials.
    if ((integration.credentials as { demo?: boolean } | null)?.demo === true) {
      const sourceKey = String(integration.type || 'crm').toLowerCase();
      const sampleDeals = [
        { id: `demo-${sourceKey}-001`, name: 'Acme Corp — Platform Renewal', amount: 120000, stage: 'Negotiation', client: 'Acme Corp' },
        { id: `demo-${sourceKey}-002`, name: 'Globex — Data Migration', amount: 45000, stage: 'Proposal Sent', client: 'Globex Inc' },
        { id: `demo-${sourceKey}-003`, name: 'Initech — Annual Support Contract', amount: 24000, stage: 'Qualification', client: 'Initech LLC' },
      ];
      ...
      const newDeals = sampleDeals.filter((d) => !existingIds.has(d.id));

      if (newDeals.length > 0) {
        const proposals = newDeals.map((deal) => ({
          id: crypto.randomUUID(),
          title: deal.name,
          content: `Deal imported from ${integration.name} (demo mode)\n\nClient: ${deal.client}\nAmount: $${deal.amount.toLocaleString()}\nStage: ${deal.stage}\n\nThis record was generated by the demo integration to showcase the CRM import flow.`,
        ...
        if (insertError) {
          syncFailed = true;
          syncDetails = { error: insertError.message, message: 'Sync failed' };
        } else {
          recordsAffected = inserted?.length || 0;
          syncDetails = {
            message: `Demo sync imported ${recordsAffected} sample deals from ${integration.name}`,
            demo: true,
        ...
        syncDetails = {
          message: 'Demo sync complete — all sample deals already imported.',
          demo: true,
          proposalsCreated: 0,
        };
      }
    } else if (integration.type === 'HUBSPOT' && integration.credentials) {
      try {
    ...
    const { data: syncLog, error: logError } = await supabase
      ...
      .insert({
        ...
        integrationId: id,
        action: 'SYNC',
        // A sync with nothing new to import is still a success, not a failure.
